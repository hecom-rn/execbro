...
- `swipe`: Cross-platform swipe/scroll gesture (auto-routes to iOS/Android). Returns `verification.meaningful`; when it is false, `warning` names the *specific* cause by probing the scroll surface under the start point — already at top, already at end, content not scrollable, wrong axis, or no scroll view there at all. The probe only runs on a no-op, so the happy path pays nothing. Use `burst:true` for overscroll/bounce detection, `verify:false, screenshot:false` for fastest path, `delta` for iOS touch step size.
