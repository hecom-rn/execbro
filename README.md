...
- **Real Multi-Touch Pinch** *(Android emulator + HarmonyOS over hdc — iOS in progress)* - A `pinch` tool that sends two genuine kernel touch contacts — through the Android emulator's multi-touch bridge, or a single `hdc shell uinput -T -m` dual-finger command on HarmonyOS — so it zooms maps, galleries, WebViews, and native views alike — it works below the app, not through React Native. Returns the same `verification.meaningful` signal as `swipe`, and refuses on unsupported targets instead of faking a result
...
| `pinch`                                 | **Real two-finger pinch-to-zoom** — Android emulator + HarmonyOS (iOS in progress)       |
