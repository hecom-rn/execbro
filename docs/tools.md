...
Cross-platform tools (`tap`, `swipe`, `input_text`, `get_logs(source="native")`, and all CDP-level tools) support HarmonyOS targets automatically once the app is connected through Metro. Limitations: pinch is unsupported; tap's strategies on harmony are fiber and accessibility (testID); the typed value cannot be verified on the native text path.
