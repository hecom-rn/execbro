...
| Tool                          | Description                                                                                                                                                                                                                                                                                                                                                                |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `navigate`   | Navigate the router directly and verify the route actually changed. Expo Router takes paths, React Navigation takes route names; unknown names are rejected before dispatch with nearest-match suggestions, and the error lists the registered routes. `routeTable=true` with no destination just lists the route names. An app written as a plain `<NavigationContainer>` with no `ref` prop is reached through a mounted screen's own navigation object instead; that path has no `getRootState`, so the route table is empty and destination validation is skipped, and the note says so |
...
| `pinch`                       | **Real two-finger pinch-to-zoom — ANDROID EMULATOR + HARMONYOS over hdc (iOS in progress).** Sends two genuine kernel touch contacts — through the emulator's multi-touch bridge on Android, or a single `hdc shell uinput -T -m` dual-finger command on HarmonyOS — so it drives any surface on screen (React Native, native views, WebViews, maps), not just RN. `direction:"out"` zooms in, `"in"` zooms out; `x`/`y` set the focal point in the shared screenshot-pixel space; `scale` is the finger-separation ratio and chains into multiple gestures when too large for one; `angle` picks the finger axis. `span` is the fraction of the screen the gesture occupies — 1 by default for `"out"`, 0.5 for `"in"` (a pinch-in starts with the fingers far apart, so a full span lands them on a top bar or bottom sheet). Lower it further if a gesture still lands on surrounding UI. Returns `verification.meaningful` like `swipe`. Physical Android devices and iOS return an explicit error rather than a partial result; on HarmonyOS `durationMs` is not applied (movement runs at the uinput default ~1s) |
...
Cross-platform tools (`tap`, `swipe`, `input_text`, `get_logs(source="native")`, and all CDP-level tools) support HarmonyOS targets automatically once the app is connected through Metro. `pinch` also works on HarmonyOS through `hdc shell uinput -T` (no Metro connection needed; `durationMs` is not applied). Remaining limitations: tap's strategies on harmony are fiber and accessibility (testID); the typed value cannot be verified on the native text path.
