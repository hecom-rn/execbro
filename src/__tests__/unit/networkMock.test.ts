...
/**
 * The mock layer lives inside the injected script, so these tests run it for
 * real in a `vm` context and drive a fake XMLHttpRequest, exactly as
 * networkInterceptorXhr.test.ts does. Asserting on the script's source text
 * would prove nothing about whether a mock is ever delivered.
 */
...
/**
 * A fresh class per sandbox: the script patches XMLHttpRequest.prototype in
 * place, so a shared class would carry one test's patch — and its closure over
 * one test's console — into the next.
 */
...
describe("buildMockPushScript", () => {
    ...
    it("a later push overwrites an earlier one rather than appending", () => {
        ...
        vm.runInContext(
            buildMockPushScript(JSON.stringify([{ id: "m2", url: "/b", mode: "replace", status: 404 }])),
            t.sandbox as unknown as vm.Context
        );
