    ...
    registerToolWithTelemetry(
        ...
        async ({ device }) => {
            ...
            if (!result.success && (result.error ?? "").includes("no focused TextInput")) {
                ...
                if (keyboard.error) {
                    // The Keyboard module is unreachable here (harmony /
                    // new-arch bundles), so "already down" cannot be proven —
                    // report honestly instead of guessing.
                    return {
                        content: [{ type: "text", text: `No TextInput had focus, so there was nothing to blur. Keyboard visibility could NOT be verified (${keyboard.error}).` }]
                    };
                }
                if (!keyboard.visible) {
                    return {
                ...
                return {
                    content: [{
                        ...
                        text: "Error: the keyboard is up but no React TextInput has focus, so blurring React cannot close it." +
                            ...
                            " Dismiss it from the platform instead: android_key_event({ key: \"BACK\" }), harmony_key_event({ key: \"BACK\" }), or tap outside the field on iOS."
                    }],
                    isError: true
                };
            }
